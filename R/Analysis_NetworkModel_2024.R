...
scaled_TQ_df <- readRDS("data/final_lmer_model_network/scaled_TQ_df.RDS")
...
for(scen in scen_temp){
  rm(list=setdiff(ls(), c(intial_objects, "scen", "intial_objects"))) #remove everything in runs before 
    ...
    ##########################
    #Read in Daily Flow 
    #from "Calculate_Qscaling_v2.R#
    net_lstQ <- readRDS("output/data/net_lst_baseQ_v4.RDS")
    #net_lstQ <- readRDS("output/data/net_lst_FlowQ_v4.RDS")#total streamflow
    
    
    temp_sin <- scen_T[scen][[1]]
    
    #Add Temperture to Each Frame 
    ...
    #SETTING UP INITIAL DATE FOR THE RUN,
          ...
          ### INITIAL INPUT
                    ...
                    if(!exists("network_pre") || yday(t_s) != yday(ts_pre)){
                      ...
                      #Temperature now within the scenario initialization
